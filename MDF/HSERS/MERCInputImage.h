...
public:
	...
	void Read_Image(Image<T> *inputImage, int conn8, int dist); // modified
...
template <class T>
void MERCInputImage<T>::Read_Image(Image<T> *inputImage, int conn8, int dist)
{
	width_ = inputImage->width();
	height_ = inputImage->height();
	nNodes_ = width_*height_;
	...
	for (int y = 0; y < height_; y++)
	{
		for (int x = 0; x < width_; x++) 
		{
			if (x < width_-1) 
			{
				...
				edges_[num].b_ = y * width_ + (x+1);
				if (dist == 0) 
					edges_[num].w_ = cosine(inputImage->Access(x,y), inputImage->Access(x+1,y));
				else if (dist == 2) 
					edges_[num].w_ = euclid(inputImage->Access(x,y), inputImage->Access(x+1,y));
				else 
					edges_[num].w_ = abs(1.0*(inputImage->Access(x,y) - inputImage->Access(x+1,y)));
				num++;
			}
			...
			if (y < height_-1) 
			{
				...
				edges_[num].b_ = (y+1) * width_ + x;
				if (dist == 0) 
					edges_[num].w_ = cosine(inputImage->Access(x,y), inputImage->Access(x,y+1));
				else if (dist == 2) 
					edges_[num].w_ = euclid(inputImage->Access(x,y), inputImage->Access(x,y+1));
				else 
					edges_[num].w_ = abs(1.0*(inputImage->Access(x,y) - inputImage->Access(x,y+1)));
				num++;
			}
			...
			if(conn8==1)
			{
				if ((x < width_-1) && (y < height_-1)) 
				{
					...
					edges_[num].b_ = (y+1) * width_ + (x+1);
					if (dist == 0) 
						edges_[num].w_ = sqrt(2.0)*cosine(inputImage->Access(x,y), inputImage->Access(x+1,y+1));
					else if (dist == 2) 
						edges_[num].w_ = sqrt(2.0)*euclid(inputImage->Access(x,y), inputImage->Access(x+1,y+1));
					else 
						edges_[num].w_ = sqrt(2.0)*abs(1.0*(inputImage->Access(x,y) - inputImage->Access(x+1,y+1)));
					num++;
				}
